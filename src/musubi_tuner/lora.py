...
class LoRAModule(torch.nn.Module):
  ...
  def __init__(
    ...
    if split_dims is None:
      ...
      self.lora_down = torch.nn.ModuleList([torch.nn.Linear(in_dim, self.lora_dim, bias=False) for _ in range(len(split_dims))])
...
class LoRANetwork(torch.nn.Module):
  """LoRA over the DiT (U-Net) only — text encoders are not adapted."""
  ...
  def __init__(
    ...
    def create_modules(
      ...
      target_replace_mods: list[str] | None = None,
    ...
    # create LoRA for U-Net (DiT) — text encoders are not adapted
    self.unet_loras: list[LoRAModule | LoRAInfModule]
    self.unet_loras, skipped = create_modules(True, prefix, unet, target_replace_modules)
  ...
  def apply_to(self, unet: nn.Module | None):
    if len(self.unet_loras) == 0:
      logger.error(
        "No LoRA modules. Please check `--network_args` / LoRAモジュールがありません。`--network_args`を確認してください"
      )
  ...
  def merge_to(self, unet, weights_sd, dtype=None, device=None, non_blocking=False):
...
def create_network_from_weights(
  ...
  module_class: type[object] | None = None,
  module_kwargs: dict[str, Any] | None = None,
  **kwargs,
) -> LoRANetwork:
