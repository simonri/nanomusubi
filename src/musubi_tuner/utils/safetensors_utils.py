...
def load_safetensors(
  ...
  if disable_mmap:
    ...
    try:
      ...
      # device may be invalid for safetensors.load_file; fall back to default device
...
def get_split_weight_filenames(file_path: str) -> list[str] | None:
  """
...
class WeightTransformHooks:
  split_hook: Callable[..., Any] | None = None


class TensorWeightAdapter:
  """Wraps MemoryEfficientSafeOpen to apply a split_hook when loading tensors.

  split_hook: (original_key, original_tensor) -> (new_keys, new_tensors).
  If tensors is None, return only new keys (no tensors).
  """

  def __init__(self, weight_convert_hook: WeightTransformHooks, original_f: MemoryEfficientSafeOpen):
    self.original_f = original_f
    self.new_key_to_original_key_map: dict[str, str] = {}
    self.new_keys = []
    self.tensor_cache = {}
  ...
  def get_tensor(self, new_key: str, device: torch.device | None = None, dtype: torch.dtype | None = None) -> torch.Tensor:
